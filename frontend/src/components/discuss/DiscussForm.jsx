...
const DiscussForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!user) {
            setErrorMessage('You must be signed in to create a topic.');
            toast.error('You must be signed in to create a topic.');
            return;
        }

        try {
            // 1. Add topic to Firestore with author's name and image
            const topicRef = await addDoc(collection(db, 'topics'), {
                ...formData,
                author: authorName,
                authorImage: authorImage,
                authorId: user.uid, // Store the user ID for better reference
                authorEmail: user.email,
                createdAt: new Date(),
                comments: [], // Initialize with empty comments array
            });

            // 2. Update the user's document to track this activity
            ...
            await updateDoc(userDocRef, {
                topics: arrayUnion({
                    ...
                    activity: "created",
                    timestamp: new Date(),
                    type: formData.type || "General Discussion",
                    previewContent: formData.content.replace(/<[^>]*>/g, '').substring(0, 100) // Store a text preview without HTML
                })
            });
            ...
            // Optional: Navigate to the discussion list or the new topic
            setTimeout(() => {
                navigate('/discussion');
            }, 1500);
